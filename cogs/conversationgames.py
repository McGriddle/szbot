...
class ConversationGames(commands.Cog):
    ...
    @commands.command()
    @commands.bot_has_permissions(embed_links=True)
    async def truth(self, ctx, *, user: discord.Member = None):
        """Ask a truth question to users!"""

        if not user:
            return await ctx.send("You must mention someone.")
        # Set author
        author = ctx.message.author

        # Get and pick random string
        strings = self.questions['truths']
        mn = len(strings)
        rs = randint(0, mn - 1)
    ...
    @commands.command()
    @commands.bot_has_permissions(embed_links=True)
    async def dare(self, ctx, *, user: discord.Member = None):
        """Dare someone!"""
